...
export default function DemandesPage() {
  ...
  const handleResponseSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Réponse envoyée avec succès ! Le dossier est de nouveau en attente de validation RH.");
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      ...
      {/* --- ACTION REQUIRED SECTION (High Visibility) --- */}
      {requests.some(r => r.statutCycleVie === "ATTENTE") && (
        <div className="mb-10 animate-in slide-in-from-top duration-500">
           <div className="bg-amber-100 border-2 border-amber-200 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-amber-500/10">
              <div className="flex items-center gap-6">
                 <div className="w-16 h-16 bg-amber-500 rounded-3xl flex items-center justify-center text-white shadow-lg animate-bounce">
                    <AlertCircle size={32} />
                 </div>
                 <div>
                    <h3 className="text-xl font-black text-amber-900 uppercase italic tracking-tight">Action requise sur vos dossiers</h3>
                    <p className="text-amber-700 text-sm font-bold opacity-80">Le service RH a mis certains de vos dossiers en attente d'informations complémentaires.</p>
                 </div>
              </div>
              <div className="flex gap-4">
                 <span className="bg-white/50 px-4 py-2 rounded-xl text-amber-800 font-black text-xs border border-amber-200">
                    {requests.filter(r => r.statutCycleVie === "ATTENTE").length} En suspens
                 </span>
              </div>
           </div>
        </div>
                ...
                <td className="p-8 text-center flex flex-col items-center justify-center gap-2">
                  ...
                  {req.statutCycleVie === 'ATTENTE' && (
                      <button 
                         ...
                         className="bg-amber-500 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition shadow-lg flex items-center gap-2"
                      >
      ...
      {/* --- RESPONSE MODAL (With File Upload) --- */}
      {responseRequest && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex justify-center items-center z-[110] p-4">
           <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
             <div className="bg-amber-500 p-10 text-white flex items-center justify-between">
                <div>
                   <h3 className="text-2xl font-black uppercase italic tracking-tighter">Répondre au service RH</h3>
                   <p className="text-amber-100 text-xs font-bold mt-1 uppercase tracking-widest">Dossier #{responseRequest.idRequete}</p>
                </div>
                <button onClick={() => setResponseRequest(null)} className="p-2 hover:bg-white/10 rounded-full transition"><X size={24} /></button>
             </div>
             
             <div className="p-10 space-y-8">
                <div className="bg-amber-50 border border-amber-100 p-6 rounded-3xl">
                   <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2">Message du Service RH</p>
                   <p className="text-sm font-bold text-gray-700 italic">"{responseRequest.commentaireAction || "Merci de nous fournir des pièces complémentaires."}"</p>
                </div>

                <div className="space-y-4">
                   <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest pl-2">Votre Réponse / Explications</label>
                   <textarea 
                     className="w-full bg-gray-50 border-2 border-gray-100 rounded-3xl p-6 font-bold text-gray-700 focus:border-amber-500 outline-none transition-all h-32 resize-none"
                ...
                <div className="space-y-4">
                   <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest pl-2">Attacher un justificatif (Optionnel)</label>
                   <div className="relative group">
                      ...
                      />
                      <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl p-8 flex flex-col items-center justify-center gap-4 group-hover:border-amber-400 group-hover:bg-amber-50/50 transition-all">
                         <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-gray-400 shadow-sm group-hover:text-amber-500 transition-colors">
                            <Download size={24} />
                         </div>
                         <div className="text-center">
                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{responseFile ? responseFile.name : "Cliquez ou glissez un fichier"}</p>
                            <p className="text-[9px] text-gray-400 font-bold mt-1 uppercase">PDF, JPG ou PNG (Max 5Mo)</p>
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-gray-900 text-white py-6 rounded-3xl font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl disabled:opacity-30 disabled:grayscale flex items-center justify-center gap-4"
                >
                  {isSubmittingResponse ? "Envoi en cours..." : "Soumettre la réponse"}
                </button>
             </div>
           </div>
        </div>
