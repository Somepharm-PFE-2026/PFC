          ...
          try {
              ...
              if (res.ok) {
                  ...
                  if (data.status === "ENVOYÉ" || data.status === "EN_ATTENTE_EMPLOYÉ") {
                      // 🛡️ HARDENED: Check both pathname and full URL to be absolutely sure
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.path}
              className={`flex items-center rounded-2xl font-black text-sm transition-all duration-300
            ...
            >
              ...
              {!isSidebarRetracted && (
                <div className="flex-1 flex items-center justify-between animate-in fade-in slide-in-from-left-2 duration-300">
                  ...
                  {(item.badge || 0) > 0 && (
                    <span className="bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-lg shadow-red-200">
                      {item.badge}
                    </span>
                  )}
                </div>
