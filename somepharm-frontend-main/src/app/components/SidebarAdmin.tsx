...
export default function SidebarAdmin() {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950/70 backdrop-blur-md border-b border-yellow-500/10 z-40 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
           <User size={20} className="text-yellow-400/80" />
        </div>
      </div>
      ...
      <div className={`fixed top-0 bottom-0 left-0 lg:top-6 lg:bottom-6 lg:left-6 bg-gradient-to-r from-yellow-300 via-yellow-400 to-yellow-600 text-white shadow-2xl lg:shadow-[0_12px_40px_rgba(234,179,8,0.3)] border-r lg:border border-white/20 lg:rounded-[3rem] p-6 pt-6 flex flex-col z-50 overflow-hidden transition-all duration-700 ease-in-out
        ${isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
        ${isSidebarRetracted ? "lg:w-28 shadow-none border-white/10" : "w-[280px] lg:w-80 shadow-gray-200/40"}
      ...
      <div className="relative h-28 flex flex-col items-center justify-center pt-2 mb-4">
        ...
        {!isSidebarRetracted && (
          <button 
            ...
            className="absolute -right-2 top-8 p-2 bg-yellow-800 border border-yellow-600 rounded-xl text-yellow-200 hover:text-white hover:bg-yellow-700 hover:shadow-lg transition-all z-20 group"
          >
          ...
          return (
            <Link
              ...
              className={`flex items-center rounded-2xl font-bold text-sm transition-all duration-300 relative group
                ...
                ${isActive 
                  ? "bg-white text-yellow-800 shadow-lg shadow-black/10 hover:translate-x-1" 
                  : "text-yellow-100 hover:bg-white/10 hover:text-white hover:shadow-sm hover:translate-x-1"}`}
            >
      ...
      <div className="pt-6 mt-2 border-t border-white/20 shrink-0">
        ...
        <button
          ...
          className={`w-full flex items-center text-red-200 font-bold text-sm hover:bg-red-500/20 hover:text-red-100 rounded-2xl transition-all
            ${isSidebarRetracted ? "justify-center p-4" : "gap-4 px-6 py-4"}`}
        >
