...
export default function SidebarSuperAdmin() {
    ...
    return (
        <>
            {/* Mobile Top Bar */}
            <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-[#050505] border-b border-white/[0.05] z-40 flex items-center justify-between px-4">
                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => setIsMobileOpen(true)}
                        className="p-2 -ml-2 text-gray-400 hover:bg-white/5 rounded-lg transition-colors"
                    >
                        <Menu size={24} />
                    </button>
                    <h1 className="text-lg font-black text-white tracking-tight uppercase italic leading-none">Some<span className="text-red-600">Admin</span></h1>
                </div>
            ...
            <aside className={`fixed top-0 bottom-0 left-0 lg:sticky lg:top-0 w-[280px] lg:w-80 h-screen bg-[#050505] text-gray-400 flex flex-col overflow-y-auto no-scrollbar border-r border-white/[0.03] shadow-[25px_0_50px_-20px_rgba(0,0,0,0.8)] z-50 transition-all duration-500 ease-in-out
                ${isMobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
            `}>
                {/* Mobile Close Button */}
                <button 
                    onClick={() => setIsMobileOpen(false)}
                    className="lg:hidden absolute top-4 right-4 p-2 text-gray-400 hover:bg-white/10 hover:text-white rounded-lg transition-colors z-50"
                >
                    <X size={20} />
                </button>
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`group flex items-center gap-4 px-6 py-4 rounded-xl transition-all duration-500 relative overflow-hidden ${
                                            isActive 
                                                ? "bg-gradient-to-r from-teal-400 to-teal-500 text-white shadow-[0_15px_40px_-10px_rgba(20,184,166,0.5)] translate-x-2" 
                                                : "hover:bg-white/[0.02] hover:text-white hover:translate-x-1"
                                        }`}
                                    >
