    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
           <button className="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm hover:shadow-md transition-all text-gray-400 hover:text-blue-600">
              <Download size={20} />
           </button>
                         ...
                         <tr key={log.id} className="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td className="px-10 py-6">
                               <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                 log.typePointage === 'ENTREE' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'
                               }`}>
                                  {log.typePointage}
                               </span>
                            </td>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 relative">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="bg-white p-4 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4">
                   <Calendar size={20} className="text-blue-600 ml-4" />
                   <input 
                     type="date" 
                     value={selectedDate}
                     onChange={(e) => setSelectedDate(e.target.value)}
                     className="bg-transparent border-none outline-none font-black text-xs uppercase tracking-widest text-gray-800 p-4"
                   />
                </div>
             </div>
                          ...
                          <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                             ...
                             <td className="px-10 py-6">
                                <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
                                  log.typePointage === 'ENTREE' ? 'bg-emerald-50 text-emerald-600' : 
                             ...
                             <td className="px-10 py-6 text-right">
                                <span className={`font-black text-[9px] uppercase ${
                                  log.statut === 'OK' ? 'text-emerald-500' : log.statut === 'REGULARISE' ? 'text-blue-500' : 'text-amber-500'
                                }`}>
                                   {log.statut}
                                </span>
                             </td>
                                 ...
                                 <tr key={i} className="hover:bg-gray-50/30 transition-colors">
                                    ...
                                    <td className="px-10 py-6 text-right">
                                       <span className={`px-4 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
                                          day.status === 'OK' ? 'bg-emerald-50 text-emerald-600' :
          ...
          <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500 pb-20">
             ...
             <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className="bg-white border-2 border-gray-100 p-4 rounded-2xl hover:bg-gray-50 transition-all shadow-sm"
                >
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="bg-white p-10 rounded-[4rem] border border-gray-100 shadow-xl space-y-8">
                   <div className="flex items-center justify-between">
                      <h4 className="text-sm font-black text-gray-800 uppercase italic tracking-tight">Taux d'Absentéisme (%)</h4>
                      <span className="text-[10px] font-black text-emerald-500 uppercase">Tendance Stable</span>
                   </div>
                   <div className="h-64 flex items-end justify-between gap-4 px-4">
                      {analyticsData && Object.entries(analyticsData.absenteeismTrend).map(([month, val]: any, i) => (
                         <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                            <div 
                              className="w-full bg-blue-100 rounded-2xl group-hover:bg-blue-600 transition-all duration-500 relative"
                              style={{ height: `${val * 15}px` }}
                            >
                               <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-[10px] font-black text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">
                                  {val}%
                               </span>
                            </div>
                ...
                {/* LATENESS BY DEPT */}
                <div className="bg-white p-10 rounded-[4rem] border border-gray-100 shadow-xl space-y-8">
                   <h4 className="text-sm font-black text-gray-800 uppercase italic tracking-tight">Volume de Retards par Département</h4>
                   <div className="space-y-6">
                      {analyticsData && Object.entries(analyticsData.latenessByDept).map(([dept, count]: any, i) => {
                         const max = Math.max(...Object.values(analyticsData.latenessByDept) as number[]);
                         const pct = (count / max) * 100;
                         return (
                         <div key={i} className="space-y-2">
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                               <span className="text-gray-400">{dept}</span>
                               <span className="text-gray-800">{count} retards</span>
                            </div>
                            <div className="h-3 bg-gray-50 rounded-full overflow-hidden">
                               <div 
                                 className={`h-full rounded-full transition-all duration-1000 ${i === 0 ? 'bg-amber-500' : 'bg-blue-500'}`}
                                 style={{ width: `${pct}%` }}
                               ></div>
                            </div>
                         </div>
             ...
             <div className="bg-gray-900 rounded-[4rem] p-12 shadow-2xl space-y-10 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 relative">
                   ...
                   <button className="bg-white text-gray-900 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all">
