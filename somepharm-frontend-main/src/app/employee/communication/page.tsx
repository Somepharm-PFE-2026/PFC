    ...
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-gradient-to-br from-sky-50/80 via-white to-blue-50/30 border border-sky-100/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-6 lg:p-8 overflow-hidden mb-2">
        ...
        <div className="relative z-10 border-l-4 border-sky-500 pl-5">
          <h1 className="text-3xl lg:text-4xl font-heading font-black italic text-slate-800 tracking-tight drop-shadow-sm uppercase">
            Fil <span className="text-sky-600">d'Actualités</span>
          </h1>
        ...
        <div className="relative z-10 bg-white/60 backdrop-blur-md p-4 rounded-2xl hidden sm:block border border-sky-100/50 shadow-sm">
           <Megaphone size={32} className="text-sky-600" />
        </div>
      </div>
         ...
         {annonces.length > 0 ? annonces.map((annonce: any) => (
           <div 
             ...
             className={`bg-white rounded-2xl border shadow-sm p-5 lg:p-6 transition-all hover:shadow-md relative flex flex-col md:flex-row gap-4 lg:gap-6 cursor-pointer group
               ...
               ${annonce.isRead ? "opacity-70" : "ring-2 ring-sky-500/10 border-sky-100"}`}
           >
              <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-3">
                     ...
                     {annonce.attachmentUrl && (
                        <span className="text-sky-600 flex items-center gap-1 text-[9px] font-bold uppercase bg-sky-50 px-2 py-0.5 rounded">
                  ...
                  <div className="flex items-center gap-2">
                     <div className="w-6 h-6 bg-slate-100 rounded-full flex items-center justify-center font-bold text-sky-600 text-[10px]">
              ...
              <div className="flex md:flex-col justify-between items-center md:items-end gap-3" onClick={(e) => e.stopPropagation()}>
                  {!annonce.isRead ? (
                    <button 
                      ...
                      className="bg-sky-600 text-white px-4 py-2.5 rounded-xl shadow-sm flex items-center gap-2 hover:bg-sky-700 active:scale-95 transition-all font-bold text-[10px] uppercase"
                    >
                       <CheckCircle2 size={14} /> Lu
                    </button>
                  ) : (
                    <div className="text-sky-600 flex items-center gap-1.5 font-bold text-[10px] uppercase px-3 py-1.5 bg-sky-50 rounded-lg border border-sky-100">
                       <CheckCircle2 size={12} /> Consulté
                    </div>
                  )}

                  <div className="p-3 bg-slate-50 text-slate-400 rounded-xl group-hover:text-sky-600 group-hover:bg-sky-50 transition-all hidden md:block">
            ...
            <div className={`flex-1 space-y-6 ${showPreview ? 'lg:max-w-sm' : ''}`}>
              ...
              {selectedAnnonce.attachmentUrl && (
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-sky-600 shadow-sm border border-slate-100">
                    <FileText size={20} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <button 
                      ...
                      className="text-sky-600 text-[10px] font-bold uppercase hover:underline"
                    >
                  ...
                  <a 
                    ...
                    target="_blank"
                    className="p-2.5 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-all"
                  >
                    <Download size={18} />
                  </a>
                </div>
              ...
              <div className="pt-6 border-t border-slate-100 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center font-bold text-sky-600 text-xs">
