  ...
  return (
    <div className="w-full bg-slate-900 rounded-2xl lg:rounded-3xl p-4 sm:p-6 lg:p-10 text-white relative overflow-hidden flex flex-col items-center min-h-[70vh] lg:min-h-[80vh]">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_0%,_#0284c7_0%,_transparent_50%)]"></div>
        <div className={`absolute bottom-0 right-0 w-[500px] h-[500px] blur-[120px] rounded-full transition-all duration-1000
          ${scanResult?.isValid === true ? 'bg-emerald-500/40' : scanResult?.isValid === false ? 'bg-rose-500/40' : 'bg-sky-500/10'}`}></div>
      </div>
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-heading font-bold uppercase tracking-tight">
              Portail <span className="text-sky-500">Sécurité</span>
            </h1>
        ...
        <div className="flex items-center justify-center p-1 bg-slate-800/50 rounded-xl border border-slate-700 w-full max-w-sm mx-auto">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all
              ${inputMode === "camera" ? "bg-sky-600 text-white shadow-md shadow-sky-900/40" : "text-slate-400 hover:text-slate-200"}`}
          >
                ...
                {cameraActive && (
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    <div className="relative w-48 h-48 sm:w-64 sm:h-64">
                      <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-sky-400 rounded-tl-xl"></div>
                      <div className="absolute top-0 right-0 w-8 h-8 border-t-2 border-r-2 border-sky-400 rounded-tr-xl"></div>
                      <div className="absolute bottom-0 left-0 w-8 h-8 border-b-2 border-l-2 border-sky-400 rounded-bl-xl"></div>
                      <div className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-sky-400 rounded-br-xl"></div>
                      <div className="absolute left-4 right-4 top-1/2 h-0.5 bg-sky-400/50 shadow-[0_0_10px_rgba(14,165,233,0.5)] animate-bounce"></div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <form onSubmit={handleScanSubmit}>
                  <input
                    ...
                    className="w-full bg-slate-900 border-2 border-slate-700 rounded-xl px-4 py-4 text-center text-lg font-mono text-sky-400 focus:outline-none focus:border-sky-500 transition-all placeholder:text-slate-700"
                    autoFocus
                  />
                </form>
