...
export default function DemandeModal({ isOpen, onClose, onSuccess, token, initialCategory }: DemandeModalProps) {
  ...
  const [typeConges, setTypeConges] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (token) {
      fetch("http://localhost:8080/api/demandes/types", {
        headers: { Authorization: `Bearer ${token}` }
      }).then(r => r.ok ? r.json() : []).then(setTypeConges).catch(() => {});
    }
  }, [token]);

  const [formDataConge, setFormDataConge] = useState({
    typeCongeId: "" as string | number,
    dateDebut: "",
    dateFin: "",
    motif: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (category === "CONGE") {
        ...
        const payload = {
          typeConge: { idTypeConge: Number(formDataConge.typeCongeId) },
          dateDebut: formDataConge.dateDebut,
          dateFin: formDataConge.dateFin,
          motif: formDataConge.motif,
        };
        ...
        if (res.ok) {
          onSuccess();
          onClose();
          setFormDataConge({ typeCongeId: "", dateDebut: "", dateFin: "", motif: "" });
        } else {
             ...
             <>
                <div>
                  ...
                  <select
                    ...
                    value={formDataConge.typeCongeId}
                    onChange={(e) => setFormDataConge({ ...formDataConge, typeCongeId: e.target.value })}
                    required
                  >
                    <option value="">Sélectionner un type...</option>
                    {typeConges.map((tc: any) => (
                      <option key={tc.idTypeConge} value={tc.idTypeConge}>{tc.nom}</option>
                    ))}
                  </select>
                </div>
