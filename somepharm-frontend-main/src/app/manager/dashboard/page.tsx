...
const PunchWidget = ({ pointage, handlePointage, loadingPointage, timeString }: any) => {
  ...
  return (
    <div className="bg-slate-900 rounded-3xl p-6 lg:p-8 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 lg:gap-8 border border-slate-800 relative overflow-hidden group">
      <div className="absolute -right-20 -top-20 opacity-[0.03] text-white group-hover:rotate-12 transition-transform duration-1000">
        <Fingerprint size={300} />
      </div>
      <div className="flex items-center gap-4 lg:gap-6 z-10 w-full md:w-auto">
        <div className="bg-slate-800 p-4 rounded-2xl text-teal-400 shadow-inner group-hover:scale-105 transition-transform shrink-0">
          <Timer size={32} />
        </div>
        <div>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mb-1">Pointeur Personnel</p>
          <h2 className="text-4xl lg:text-5xl font-bold text-white tracking-tight tabular-nums">{timeString}</h2>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          {/* PRESENCE HUD */}
          <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center">
          ...
          <div className="lg:col-span-2 bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
             ...
             <div key={weekIndex} className="grid grid-cols-5 gap-2 lg:gap-4 transition-all duration-700 animate-in fade-in slide-in-from-right-8 ease-out">
                {currentWeek.map((day: any, i: number) => (
                   <div key={i} className={`flex flex-col items-center justify-center p-4 lg:p-6 rounded-xl lg:rounded-2xl border transition-all duration-500 ${
                      day.isCritical ? 'bg-rose-50 border-rose-200 text-rose-600' : 
                      day.absenceCount > 0 ? 'bg-amber-50 border-amber-200 text-amber-700' : 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group">
          ...
          <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-slate-100">
             ...
             <div className="p-5 bg-teal-50 rounded-xl border border-teal-100 flex items-center justify-between group transition-colors hover:bg-teal-100/50">
                <div>
                ...
                <Link href="/manager/gestion-demandes" className="bg-teal-600 text-white p-3 rounded-xl hover:bg-teal-700 transition-all shadow-md group-hover:scale-110 active:scale-95"><ArrowRight size={20} /></Link>
             </div>
          </div>
      </div>
    </div>
