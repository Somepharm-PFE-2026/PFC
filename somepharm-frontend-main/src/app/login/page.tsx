...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        if (role === "SECURITY_AGENTS") router.push("/scanner");
        else if (role === "EMPLOYE") router.push("/employee/dashboard");
  ...
  return (
    <main 
      ...
      style={{
        backgroundImage: 'url("/bg-pharmacy.png")',
    ...
    >
      ...
      <div className="relative w-full max-w-[450px] rounded-2xl bg-white/95 backdrop-blur-xl p-6 sm:p-10 shadow-2xl border border-white/20 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out z-10">
        
        <div className="mb-8 text-center flex flex-col items-center">
          <div className="h-24 w-auto flex items-center justify-center mb-2">
            <img 
              src="/logo-somepharm.png" 
              alt="SomePharm Logo" 
              className="h-full object-contain drop-shadow-sm" 
            />
          </div>
          <span className="px-3 py-1 bg-teal-50 text-teal-700 rounded-full text-xs font-medium tracking-wide border border-teal-100 mt-2">
