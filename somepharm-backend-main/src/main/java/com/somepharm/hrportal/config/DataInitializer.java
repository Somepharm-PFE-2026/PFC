    ...
    CommandLineRunner initUsers(com.somepharm.hrportal.repository.UtilisateurRepository userRepo, 
        ...
        return args -> {
            if (userRepo.findByMatricule("SP-HRMGR").isEmpty()) {
                ...
                hrManager.setNom("Manager");
                hrManager.setPrenom("HR");
                hrManager.setEmail("hrmanager@somepharm.dz");
                hrManager.setMotDePasse(encoder.encode("password123"));
                hrManager.setRole(hrManagerRole);
                hrManager.setStatutCompte("ACTIF");
                hrManager.setMustChangePassword(false);
                hrManager.setSoldeConges(30.0);
                
                userRepo.save(hrManager);
                System.out.println("✅ USER: SP-HRMGR (HR Manager) created with password: password123");
            }
        };
    }
